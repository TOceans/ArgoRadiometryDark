...
for i=1:length(data380{:,1}) %analysis is performed a profile at a time
    ...
    Ed380=cell2mat(data380{i,ind(13)})*100;  %miltiply by 100 for comparison with Organelli's units
...
load GPARcorsDAY.mat % WHATEVER DAY CORRECTIONS ARE SAVED AS 412,490...
Dcors = DPARcors;
load GPARcorsNight.mat % WHATEVER NIGHT CORRECTIONS ARE SAVED AS
Ncors = GPARcors;
%% 
out = out380Ts;
...
%% 8 11 13
subplot(311)
histogram(outc(:,8),'binwidth',20)
xlim([-100 1000])
ax = gca; set(gca,'linewidth',2,'fontsize',14);
xlabel(['PAR Raw' num2str(length(outc))])

subplot(312)
histogram(outc(:,11),'binwidth',20)
xlim([-100 1000])
ax = gca; set(gca,'linewidth',2,'fontsize',14);
xlabel(['PAR Corrected Night' num2str(length(outc))])

subplot(313)
histogram(outc(:,13),'binwidth',20)
xlim([-100 1000])
ax = gca; set(gca,'linewidth',2,'fontsize',14);
xlabel(['PAR Corrected Day' num2str(length(outc))])

%% near-surface value par
bbb = find(outc(:,4) < 20);

subplot(311)
histogram(outc(bbb,8),'binwidth',1000)
xlim([-20 100000])
ax = gca; set(gca,'linewidth',2,'fontsize',14);
xlabel(['PAR Raw n =' num2str(length(outc))])
title('pres 0-20dbar')

subplot(312)
histogram(outc(bbb,11),'binwidth',1000)
xlim([-20 100000])
ax = gca; set(gca,'linewidth',2,'fontsize',14);
xlabel(['PAR Corrected Night n =' num2str(length(outc))])

subplot(313)
histogram(outc(bbb,13),'binwidth',1000)
xlim([-20 100000])
ax = gca; set(gca,'linewidth',2,'fontsize',14);
xlabel(['PAR Corrected Day n =' num2str(length(outc))])
%% FUNCTIONS
