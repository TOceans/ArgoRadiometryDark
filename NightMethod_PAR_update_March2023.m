...
KKK=0; %lat or long not in real bounds1
...
for i=1:length(data490{:,1}) %analysis is performed a profile at a time
    ...
    Ed490=cell2mat(data490{i,ind(13)});  %output in umol quanta m^-2 s^-1
                            % ^^^^^ !!! index is wavelength specific 14=380
                            % 15 = 412 17 = 490 13 = PAR
...
% %% TEST distribution of par for p > 300dbar
% 
% a = find(outNight490(:,4) > 300);
% dpar = outNight490(a,7);
% 
% histogram(dpar,'binwidth',10)
% xlim([-50 50])
% ax = gca; set(gca,'linewidth',2,'fontsize',14);
% xlabel(['Dark PAR Night, n = ' num2str(length(dpar))])
% text (10, 2000, ['mean = ' num2str(nanmean(dpar))])
% text (20, 3000, ['median = ' num2str(nanmedian(dpar))])
% title('for p > 300 dbar')
% %% 
% b = find(outNight490(:,4) > 300 & outNight490(:,7) > 30);
% idbad = outNight490(b,8);
% %%
% hist(idbad)
...
% %% histogram of "dark" values
% subplot(211)
% histogram(darkout(:,7),'binwidth',.01)
% xlim([-0.2 0.2])
% ax = gca; set(gca,'linewidth',2,'fontsize',14);
% xlabel(['Dark \lambda = 490 Day, n =' num2str(length(darkout))])
% %% 
% %subplot(212)
% histogram(outNight490(:,7),'binwidth',.01)
% %xlim([-0.2 0.2])
% ax = gca; set(gca,'linewidth',2,'fontsize',14);
% xlabel(['Dark \lambda = 490 Night, n = ' num2str(length(outNight490))])
...
% %% 
% histogram(ns(:,8),'binwidth',10)
% xlim([-50 300])
% ax = gca; set(gca,'linewidth',2,'fontsize',14);
% xlabel(['Dark PAR Night, n = ' num2str(length(ns))])
% text (200, 20000, ['mean = ' num2str(nanmean(ns(:,8)))])
% text (200, 30000, ['median = ' num2str(nanmedian(ns(:,8)))])
% title('After log-linear test')
